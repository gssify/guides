...
本文档基于[百度FEX团队规范](https://github.com/fex-team/styleguide)做了微调。

本文档目标是使HTML代码风格保持一致，容易被理解和被维护。
...
#### [强制] `class` 必须单词全字母小写，使用 `BEM` 进行样式的组织。
...
如果只是一个临时的活动页面，可以使用`<style>......</style>`
...
```html
<!-- good -->
<li class="{% if item.type_id == current_type %}focus{% endif %}">{{ item.type_name }}</li>

<!-- bad -->
<li {% if item.type_id == current_type %} class="focus"{% endif %}>{{ item.type_name }}</li>
```
